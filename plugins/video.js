const yts = require('yt-search');
const axios = require('axios');

module.exports = {
  command: 'video',
  alias: ["ytmp4", "mp4", "ytv", "vi", "v", "vid", "vide", "videos", "ytvi", "ytvid", "ytvide", "ytvideos", "searchyt", "download", "get", "need", "search"],
  description: "Download YouTube videos in MP4 format",
  category: "download",
  react: "🥺",
  usage: ".video <video name>",
  execute: async (socket, msg, args) => {
    const sender = msg.key.remoteJid;
    const text = args.join(" ");

    if (!text) {
      return await socket.sendMessage(sender, {
        text: ` 
╭───────────────⭓
│ ❗ *Usage Example:*
│ │ 🎬 *.video <video name>*
│ │ Example:
│ 🔍 *.video Alan Walker Faded*
│ │ This command searches YouTube
│ and lets you download MP4 videos 🎞️
╰───────────────⭓
`
      }, { quoted: msg });
    }

    try {
      console.log("Searching for video...");
      const search = await yts(text);
      if (!search.videos.length) return await socket.sendMessage(sender, {
        text: ` 
╭───────────────⭓
│ 😔 *No video found!*
│ Please try again with another title.
╰───────────────⭓
`
      }, { quoted: msg });

      const video = search.videos[0];
      console.log("Video found:", video.title);

      const safeTitle = video.title.replace(/[\\/:*?"<>|]/g, '');
      const fileName = `${safeTitle}.mp4`;
      const apiURL = `https://noobs-api.top/dipto/ytDl3?link=https://www.youtube.com/watch?v=${video.videoId}&format=mp4`;

      console.log("API URL:", apiURL);
      const { data: apiRes } = await axios.get(apiURL);
      console.log("API Response:", apiRes);

      if (!apiRes?.status || !apiRes.result?.media?.video_url) {
        return await socket.sendMessage(sender, {
          text: ` 
╭───────────────⭓
│ 😔 *Failed to process video download!*
│ Please try again later.
╰───────────────⭓
`
        }, { quoted: msg });
      }

      const result = apiRes.result.media;
      const caption = ` 
╭───────────────⭓
│ 🎬 *${video.title}* 
│ 👁️ *Views:* ${video.views}
│ ⏱️ *Duration:* ${video.timestamp}
│ 📅 *Uploaded:* ${video.ago}
│ 🔗 *Link:* ${video.url}
│ 
│ 🔢 *Reply with the number to download:*
│ 
│ ╭─────────────●●►
│ ├ 🎞️ *1* — Normal Video
│ ├ 📁 *2* — As File (Document)
│ ╰─────────────●●►
╰───────────────⭓
`;

      const sentMsg = await socket.sendMessage(sender, {
        image: { url: result.thumbnail },
        caption
      }, { quoted: msg });

      console.log("Message sent, waiting for reply...");

      const messageID = sentMsg.key.id;
      let isProcessing = false;

      const listener = async (msgData) => {
        if (isProcessing) return;
        isProcessing = true;

        try {
          const receivedMsg = msgData.messages[0];
          console.log("Received message:", receivedMsg);

          if (!receivedMsg?.message) return;
          const receivedText = receivedMsg.message.conversation || receivedMsg.message.extendedTextMessage?.text;
          const isReplyToBot = receivedMsg.message.extendedTextMessage?.contextInfo?.stanzaId === messageID;
          const senderID = receivedMsg.key.remoteJid;

          if (isReplyToBot) {
            console.log("Reply detected, processing...");
            switch (receivedText.trim()) {
              case "1":
                await socket.sendMessage(senderID, {
                  video: { url: result.video_url },
                  mimetype: "video/mp4",
                  caption: ` 
╭───────────────⭓
│ ✅ *Here’s your video!*
│ Enjoy watching 🎬
│ 
│ 👑 *Downloaded by DML-MIN BOT*
╰───────────────⭓
`
                }, { quoted: receivedMsg });
                break;
              case "2":
                await socket.sendMessage(senderID, {
                  document: { url: result.video_url },
                  mimetype: "video/mp4",
                  fileName: fileName,
                  caption: ` 
╭───────────────⭓
│ ✅ *Video saved as document!*
│ Perfect for sharing 💾
│ 
│ 👑 *Downloaded by DML-MIN BOT*
╰───────────────⭓
`
                }, { quoted: receivedMsg });
                break;
              default:
                await socket.sendMessage(senderID, {
                  text: ` 
╭───────────────⭓
│ ⚠️ *Please reply only 1 or 2!*
│ Choose the correct option 🥺
╰───────────────⭓
`
                }, { quoted: receivedMsg });
            }
          }
        } catch (err) {
          console.error("Reply handler error:", err);
        } finally {
          isProcessing = false;
        }
      };

      socket.ev.on("messages.upsert", listener);
      setTimeout(() => socket.ev.off("messages.upsert", listener), 2 * 60 * 1000);
    } catch (e) {
      console.error("Main error:", e);
      await socket.sendMessage(msg.key.remoteJid, {
        text: ` 
╭───────────────⭓
│ 😔 *Something went wrong!*
│ Please try again later.
╰───────────────⭓
`
      }, { quoted: msg });
    }
  }
};

